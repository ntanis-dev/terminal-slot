...
CI pins actions to immutable commit SHAs and runs the dependency-free policy validator from the public `ntanis-dev/ci` repository before installing or executing project code. It verifies registry signatures, rejects high-severity advisories, runs scheduled fail-closed OSV scans, and creates a CycloneDX production SBOM. Renovate may propose exact updates only after the same cooldown and never automerges them.
